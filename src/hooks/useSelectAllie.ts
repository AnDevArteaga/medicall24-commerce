...
export const useSelectAllie = () => {
    const { setRegisterPurchase, product, setIdMunicipioInstitucion } = usePurchaseContext();
    ...
    const [allyProvider, setAllyProvider] = useState<
    ...
    const loadMunicipalities = async (departmentId: string) => {
        try {
            ...
            const munsFiltered = muns.filter((m) =>
    ...
    const handleSelectChange = async (
        ...
        if (name === "nombre_institucion") {
            ...
            try {
                ...
                console.error(
                    "Error obteniendo datos de la institución:",
                    error,
                );
            }
        }
    ...
    const handlePromoProduct = async (promoProduct: CodeXProduct) => {
        ...
        try {
            ...
            const dpt = dpts.find((d) =>
                Number(d.id) === Number(ally.id_departamento)
            );
            if (!dpt) throw new Error("Departamento no encontrado");

            const muns = await loadMunicipalities(dpt.id);
            const mun = muns.find((m) =>
                Number(m.id) === Number(ally.id_municipio)
            );
    ...
    const loadAllies = async () => {
        try {
            ...
            const dptsFiltered = dpts.filter((d) =>
                dptIdsFromAllies.includes(d.id)
            );
