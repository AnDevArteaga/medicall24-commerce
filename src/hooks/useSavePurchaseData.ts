...
export const useSavePurchaseData = () => {
    ...
    const userId = usePurchaseContext().userId;
    const savePurchase = async (purchaseData: registerPurchase, idMunicipio?: number | string) => {
        setIsSaving(true);
        setSaveError(null);
        ...
        try {
            ...
            if (response.status === 201 || response.status === 200) {
                ...
                return {
                    ...
                    id_transaccion: purchaseData.id_transaccion
                };
            } else {
    ...
    const saveAppointment = async (id_transaccion: string, appointment: CreateAppointmentDataProps, userId: number, id_municipio: number | string, id_compra?: number) => {
        setIsSaving(true);
        setSaveError(null);
        setSaveSuccess(false);
        try {
            const response = await registerAppointmentData(id_transaccion, appointment, userId, id_municipio, id_compra);
