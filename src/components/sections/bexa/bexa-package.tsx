...
const BexaPackage: React.FC<BexaPackageProps> = ( { onVerProducto }) => {
    ...
    <section
        id="servicio"
        className="grid grid-cols-2 md:grid-cols-1 sm:grid-cols-1 xs:grid-cols-1 sm:gap-12 xs:gap-12 gap-0 items-center px-32 py-12 md:px-14 sm:px-4 xs:px-4"
    >
        {/* Contenedor del Video */}
        <div className="relative bg-gray-100 order-1 sm:hidden xs:hidden md:order-2 sm:order-2 rounded-2xl overflow-hidden max-w-lg mx-auto shadow-lg">
            <video
        ...
        <div className="sm:space-y-12 xs:space-y-12 space-y-6 md:space-y-3 p-8  order-2 md:order-1 sm:order-1 bg-gray-100 rounded-lg shadow-lg transition-transform duration-300 md:flex md:flex-col md:items-center md:justify-center md:mb-8">
            ...
            <div className="relative bg-gray-100 order-1 hidden sm:block xs:block md:order-2 sm:order-2 xs:order-2 rounded-2xl overflow-hidden max-w-lg mx-auto shadow-lg">
                <video
                    className="w-full h-full rounded-2xl object-cover object-right"
                    src="https://medicall24.com.co/wp-content/uploads/2025/01/bexa3.mp4"
                    autoPlay
            ...
            <div className="space-y-2 flex justify-start w-full flex-col">
                {/* Servicios incluidos */}
                <p className="md:text-center text-gray-700 md:text-left text-sm md:text-2xl">
                ...
                <ul className="space-y-0 md:space-y-2">
                    ...
                    <li className="flex space-x-3 hover:translate-x-2 transition-transform duration-200">
                        ...
                        <div className="ml-2 group relative flex">
                            ...
                            <div className="absolute bottom-full left-1/2 transform translate-y-[-5px] invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all bg-gray-800 text-white text-sm p-2 rounded-lg shadow-md w-96 sm:text-xs xs:text-xs sm:w-60 sm:-translate-x-48 xs:w-60 xs:-translate-x-48">
                                <p className="text-white ml-2 text-sm md:text-base sm:text-base xs:text-sm">
                    ...
                    <li>
                        <div className="flex space-x-3 hover:translate-x-2 transition-transform duration-200">
                            ...
                            <div className="ml-2 group relative flex">
                                <Info className="w-4 h-4 md:w-6 md:h-6 text-gray-500 hover:text-pink-600 cursor-pointer" />
                                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 translate-y-[-5px] invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all bg-gray-800 text-white text-sm p-2 rounded-lg shadow-md w-96 sm:text-xs xs:text-xs sm:w-72 xs:w-56 sm:-translate-x-72 xs:-translate-x-72">
                ...
                {/* Precio */}
                <Price price="$249.500" />
                <div className="flex justify-center md:items-left flex-col w-2/3 md:w-full sm:w-full xs:w-full">
                    {/* Botón de Acción */}
                    <ButtonBuy text="Comprar Paquete" onClick={() => onVerProducto(16)} />
                </div>
                <div className="flex flex-col w-1/2 sm:w-full xs:w-full">
