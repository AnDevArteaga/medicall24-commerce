...
const StepTwo: React.FC = () => {
  ...
  const { handleValidateDomainEmail } = useRegister()
  const {
  ...
  return (
    <div className="bg-white rounded-lg p-6 lg:p-6 xl:p-6 sm:p-2 w-full grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 sm:grid-cols-1 md:grid-cols-1 gap-6 xs:p-2 xs:grid-cols-1">
      {/* Columna izquierda */}
      <div>
        <form className="space-y-2" autoComplete="off">
          ...
          <div className="flex items-center gap-2 mb-4 p-2 bg-pink-50 rounded-lg">
            <InputCheck
              id="isPacienteComprador"
              checked={isBuyerPatient}
              onChange={(e) => setIsBuyerPatient(e.target.checked)}
              label="El paciente es el comprador"
            />
          </div>

          <p className="text-xs text-gray-700 text-center">
            Para métodos de pago PSE, el tipo de identificación debe ser Cédula
            de Ciudadanía.
          </p>
          {/* Fila 1 */}
          <div className="grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 md:grid-cols-2 gap-4">
            <div>
              <SelectInput
          ...
          <div className="grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 md:grid-cols-2 gap-4">
            <InputText
              label="Nombres"
              name="names"
              value={capitalize(purchaseData.names || '')}
              obligatory
              disabled={isValidPaymentMethod}
              type="text"
              errorMessage={null}
            ...
            <InputText
              label="Apellidos"
              name="lastNames"
              value={capitalize(purchaseData.lastNames || '')}
              obligatory
              disabled={isValidPaymentMethod}
              type="text"
              errorMessage={null}
        ...
        <p className="text-xs text-gray-800 mt-4">¿Necesitas factura?</p>
        ...
        <div className="flex items-center space-x-3 p-0 bg-gray-50 mt-4">
          ...
          <p className="text-xs text-gray-700">
            ...
            <span
              className="text-primary underline cursor-pointer hover:text-primarydark font-semibold"
              onClick={() => openModal('billingData')}
            >
              click aquí
            </span>
          </p>
